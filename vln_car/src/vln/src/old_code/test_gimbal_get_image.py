            ...
            while not rospy.is_shutdown():
                ...
                if key == 0:
                    ...
                    for key, image_bytes in images_dict.items():
                        ...
                        save_image = cv2.imdecode(np.frombuffer(image_bytes['30'], np.uint8), cv2.IMREAD_COLOR)
                        save_name = f'images_1/{key}_{30}.png'
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            if command == b'send_image':
                ...
                for angle in range(0, 360, 30):
                    ...
                    _, rgb_bytes = cv2.imencode('.png', rgb_image)
                    _, depth_bytes = cv2.imencode('.png', depth_image)
                    images_dict['rgb'][str(angle)] = rgb_bytes.tobytes()
                    images_dict['depth'][str(angle)] = depth_bytes.tobytes()
                data = pickle.dumps(images_dict)
                s.sendall(data)
