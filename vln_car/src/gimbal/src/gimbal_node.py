...
class GimblaNode:
    ...
    def __init__(
        ...
        self.angle_check_pub = rospy.Publisher('gimbal_angle_check', Float32, queue_size=1)

        self.turn_sub = rospy.Subscriber('gimbal_target_angle', Float32, self.receive_angle_and_turn)
    ...
    def receive_angle_and_turn(self, msg): 
        ...
        if msg.data is not None:
            ...
            while self.get_angle_and_check(target_angle, 0.05):
    ...
    def get_angle_and_check(self, target_angle = None, acc = 0.05):
        if target_angle is not None:
            angle_range = [target_angle-acc, target_angle+acc]
        else:
            return False
        self.send_check_commands(rate_hz=2, part_flag='horizontal')
        if self.frame:
            frame = self.frame
    ...
    def send_check_commands(self, rate_hz=2, part_flag = 'horizontal'):
        while not rospy.is_shutdown():
            if part_flag == 'horizontal':
                data_frame = frame_check_angle(axis='horizontal', address=self.address)
                self.ser.write(data_frame)
                # print('horizontal check command: ', data_frame.hex().upper())
                sleep(1/rate_hz)
            elif part_flag == 'vertical':
                data_frame = frame_check_angle(axis='vertical', address=self.address)
                self.ser.write(data_frame)
                # print('vertical check command: ', data_frame.hex().upper())
                sleep(1/rate_hz)
            else:
                rospy.logwarn("Invalid part flag. Please enter 'horizontal' or 'vertical'.")
